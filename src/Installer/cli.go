...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install SYNCORD")
	var updateFlag = flag.Bool("repair", false, "Repair SYNCORD")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall SYNCORD")
	...
	if *versionFlag {
		fmt.Println("Syncord Installer Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2023 Vendicated and SYNCORD contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Syncord Installer' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest SYNCORD files...")
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, SYNCORD will likely not work.")
}
