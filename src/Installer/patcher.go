...
func init() {
	if dir := os.Getenv("SYNCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using SYNCORD_USER_DATA_DIR")
		BaseDir = dir
	} else if dir = os.Getenv("DISCORD_USER_DATA_DIR"); dir != "" {
		Log.Debug("Using DISCORD_USER_DATA_DIR/../SyncordData")
		BaseDir = path.Join(dir, "..", "SyncordData")
	} else {
		Log.Debug("Using UserConfig")
		BaseDir = appdir.New("Syncord").UserConfig()
	}
