...
func main() {
	...
	win = g.NewMasterWindow("Syncord Installer", 800, 500, 0)
...
func InstallLatestBuilds() (err error) {
	...
	if err != nil {
		ShowModal("Uh Oh!", "Failed to install the latest Syncord builds from GitHub:\n"+err.Error())
	}
	return
}
...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		g.Style().SetFontSize(20).To(
			g.Row(
				g.Style().
					...
					To(
						g.Button("Select Discord Path").OnClick(func() {
							if len(discords) > 0 {
								g.OpenURL("file://" + discords[0].(*DiscordInstall).path)
							}
						}),
					),
			),
				...
				g.Style().
					...
					To(
						g.Button("Reinstall / Repair").
							...
							Size((w-40)/3, 50),
						Tooltip("Reinstall & Update Syncord"),
					),
				g.Style().
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Syncord installed successfully by looking for its category in Discord Settings"),
		...
		InfoModal("#scuffed-install", "Hold On!", "You have a broken Discord Install.\n"+
			...
			"You need to fix this before patching, otherwise Syncord will likely not work.\n\n"+
		...
		g.Align(g.AlignRight).To(
			g.Style().SetFontSize(14).SetColor(g.StyleColorText, color.RGBA{R: 0x72, G: 0x76, B: 0x7D, A: 0xFF}).To(
				g.Label("v"+LocalVersion),
			),
		),
	}
...
func loop() {
	...
	g.PushStyleColor(g.StyleColorWindowBg, GrayBg)
	defer g.PopStyleColor()

	g.SingleWindow().
		Layout(
			g.Align(g.AlignCenter).To(
				g.Style().SetFontSize(40).To(
					g.Label("Syncord Installer"),
				),
			),

			g.Dummy(0, 10),

			&CondWidget{HasUpdate, func() g.Widget {
				return renderErrorCard(DiscordYellow, "**New update released!** Click \"Reinstall / Repair\" to update Syncord.", 40)
			}, nil},
