...
func WriteAppAsar(outFile string, patcherPath string) error {
	...
	patcherPathStr := string(patcherPathB)
	// asar format requires forward slashes, not backslashes
	patcherPathStr = strings.ReplaceAll(patcherPathStr, "\\", "/")
	indexJsContents := "require(" + patcherPathStr + ")"
