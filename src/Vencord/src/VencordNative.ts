...
export default {
    ...
    updater: {
        ...
        getLocalVersion: () => invoke<IpcRes<string>>(IpcEvents.GET_LOCAL_VERSION),
        onUpdateAvailable: (cb: (remoteVersion: string, localVersion: string) => void) => {
            ipcRenderer.on("VencordSyncordUpdate", (_, remoteVersion, localVersion) => cb(remoteVersion, localVersion));
        },
        onStatusUpdate: (cb: (status: string, local: string, remote: string, last: string) => void) => {
            ipcRenderer.on("VencordSyncordStatus", (_, status, local, remote, last) => cb(status, local, remote, last));
        },
    },
